    ...
    <main class="main-content">
        ...
        <div class="stats-container">
            <div class="stat-card">
                ...
                <div class="stat-value">€{{ number_format($revenusTotaux ?? 0, 2) }}</div>
            ...
            <div class="stat-card">
                ...
                <div class="stat-value">{{  number_format($totalBilletsVendus ?? 0, 2)  }}</div>
                <div class="stat-change">
            ...
            <div class="stat-card">
                ...
                <div class="stat-value">{{  number_format($tauxRemplissage ?? 0, 2)  }}%</div>
                <div class="stat-change">
        ...
        <div class="dashboard-actions">
                <a href="{{ route('billets.create') }}" class="btn-create-billet">➕ Créer un nouveau billet</a>
            </div>
        <!-- Table -->
        <div class="table-container">
            <table>
                ...
                <tbody>
                    @if(isset($billets) && $billets->count())
                        @foreach($billets as $billet)
                            <tr>
                                <td>{{ $billet->nom }}</td>
                                <td>€{{ number_format($billet->prix_vente, 2) }}</td>
                                <td>
                                    <form action="{{ route('billets.updateMax', $billet) }}" method="POST" class="inline-form">
                                        @csrf
                                        <input
                                            type="number"
                                            name="ventes_max"
                                            value="{{ $billet->ventes_max }}"
                                            min="1"
                                            style="width: 80px; padding: 5px; background: #1a2942; border: 1px solid #3b82f6; color: #fff; border-radius: 4px;"
                                        >
                                        <button type="submit" class="btn btn-modifier" style="padding: 5px 10px; font-size: 12px;">
                                            <i class="fas fa-edit edit-icon"></i>
                                        </button>
                                    </form>
                                </td>
                                
                                <td>{{ $billet->billets_vendus }}</td>
                                <td>€{{ number_format($billet->prix_vente * $billet->billets_vendus, 2) }}</td>
                                <td>
                                    <div class="actions">
                                        <a href="{{ route('billets.edit', $billet) }}" class="btn btn-modifier">Modifier</a>
                                        <a href="{{ route('billets.participants', $billet) }}" class="btn btn-voir">Voir Participants</a>
                                    </div>
                                </td>
